...
import { createReview } from "@/modules/plans";

type Context = { params: Promise<{ id: string; planId: string }> };

/** POST .../plans/{planId}/reviews（A113／A117：僅 active／paused 且已達 reviewDate 可建立） */
export const POST = withErrorEnvelope<Context>(async (request, requestId, context) => {
  ...
  const result = await createReview(auth.userId, id, planId);
  if (!result.ok) {
    ...
    if (result.code === "INVALID_REQUEST") {
      return apiError("INVALID_REQUEST", "計畫尚未達檢討日，或已有進行中的檢討", 409, requestId);
    }
    return apiError("NOT_FOUND", "找不到這個行動計畫。", 404, requestId);
  }

  return attachSlidingCookie(apiOk({ reviewId: result.id }, requestId), auth);
});
