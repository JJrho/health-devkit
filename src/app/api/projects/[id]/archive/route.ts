...
import { archiveProject, auditAccessDenied } from "@/modules/projects";

type Context = { params: Promise<{ id: string }> };

/** POST /api/projects/{id}/archive（AC-4）：active→archived；已封存者冪等成功 */
export const POST = withErrorEnvelope<Context>(async (request, requestId, context) => {
  ...
  const result = await archiveProject(auth.userId, id);
  if (!result.ok) {
    auditAccessDenied({
      requestId,
      userId: auth.userId,
