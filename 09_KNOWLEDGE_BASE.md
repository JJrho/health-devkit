...
### ✅ 已解決（Sprint 26，2026-08-05）

PO 提供 `og-image.jpg`（1200x630，JPEG，約 82KB，符合 OG 卡片建議尺寸與 15KB~1MB 常見上限），已存入 `public/og-image.jpg`；`src/app/page.tsx` 的 `metadata.openGraph.images` 補上 `["/og-image.jpg"]`。**過程提醒**：PO 一開始描述的檔案規格與實際找到的檔案不符（Downloads 資料夾裡有一張同名截圖，1731x909 PNG、1.28MB，明顯是聊天介面截圖而非裁切好的 OG 素材），已主動核對規格差異並詢問 PO 確認正確檔案來源（`C:\Users\jr_ho\Desktop\Medical-AI-Work\個人健康管理專案\files\og-image.jpg`），未貿然拿錯誤檔案頂上——這與 KB-026「不可自行判斷『這次應該沒關係』」同一種審慎精神：發現規格描述與實際檔案不符時，應先問不要先猜。已同步從 `KNOWN_ISSUES.md` 移除第 9 項。

## KB-038 原始掃描檔刪除：為何不做自動刪除，改用引導提示（E2-F5）

- 類型：產品決策＋個資疑慮釐清過程記錄（Sprint 26，2026-08-05，與王醫師溝通產品個資疑慮時發現的落差）
- 背景：`deleteDocument()`（`src/modules/documents/service.ts`）與對應 UI 刪除按鈕自 E2-F1（Sprint 6）就已存在且可正常運作——會清除 Storage 原始物件並軟刪除 `documents` 列。但這個功能長期「有做但沒講」：使用者完成 E2-F3 人工確認、資料正式寫入 `observations` 之後，介面上完全沒有任何提示告訴使用者「這份含姓名等資訊的原始掃描檔，其實可以自己決定要不要繼續留著」。
- 討論過的選項與為何否決自動刪除：曾考慮「確認完成後系統自動刪除原始檔」以徹底降低個資留存風險，但評估後否決，理由有二：(1) 會直接影響既有資料匯出功能（`src/modules/reports/data-export.ts` 目前會把仍存在的原始檔一併打包進匯出 ZIP，`E5-F4／C20`）——自動刪除會讓使用者事後想匯出留存副本或想重新核對原始報告時，選項已經被系統代為拿走；(2) 牴觸「數值可回溯原始報告」這條貫穿本專案的產品原則（憲法 §4「原值永遠保留」的精神延伸）——原始檔案雖不是「數值」本身，但保留它是使用者事後回頭核對數值是否抄錄正確的唯一憑據，系統不該替使用者做這個攸關信任的決定。
- 已完成處置：改為**使用者自主決定**——`document.status === "confirmed"` 時，文件卡片新增引導提示文字＋一個可聚焦既有刪除按鈕的連結（鍵盤可達），文字明確告知刪除後果（「刪除後，之後的資料匯出將不會再包含這份原始檔案，但已確認的數值本身不受影響」），讓使用者在知情的情況下自己選擇。不改動 `deleteDocument()`／`data-export.ts` 既有邏輯，純文案與導覽補充。見 `sprints/sprint-26-dor.md`。
- 影響範圍：僅 UI 層新增一段條件渲染文字＋一個 focus 按鈕，服務層與資料庫皆未變動。
- 未來避免／應用：任何「功能已存在但使用者不知道」的落差，不必然要用「系統自動幫使用者做」來補，尤其涉及使用者長期信任與資料主權（本例：要不要留著自己的原始文件）的情境——優先評估「講清楚讓使用者自己選」是否已足夠，這與本專案憲法 §3 一貫的「保守、不替使用者做醫療／個資相關決定」精神一致，只是這次的決定主體從「AI」換成「系統自動化流程」，道理相通。

## KB-039 給王醫師的技術現況說明：個資與去識別化現況（可直接轉述版本）

- 類型：技術現況彙整，供 PO 與王醫師溝通用（Sprint 26，2026-08-05）
- **結構化資料表皆無個人識別欄位**：已逐一核對 `users`（`src/db/schema/auth.ts`）、`extracted_items`（`src/db/schema/extracted-items.ts`）、`observations`（`src/db/schema/observations.ts`）等表的完整欄位定義，**沒有任何一張表有姓名、身分證字號、生日、地址、電話欄位**。`users` 表僅有 `id`（來自 Supabase Auth，非自產）、`email`、`emailVerified`、時間戳與版本欄位；`extracted_items`／`observations` 僅有檢驗項目名稱、數值、單位、參考區間、頁碼座標等純檢驗資料欄位，結構上沒有承接個人識別資訊的空間。
- **解析規則主動排除頁首病患中繼資料列**：`src/modules/extraction/parser.ts` 的 `METADATA_LINE`（A28 規則，程式碼原文 `const METADATA_LINE = /[:：]/`）會檢查候選列的「項目名稱」部分是否含半形或全形冒號——健檢報告頁首常見的「姓名：王小明」「列印序號：50065」這類病患中繼資料列，格式上必定含冒號，會被這條規則直接排除、不當作候選檢驗項目寫入 `extracted_items`。此規則的設計初衷其實是修正表格解析的假陽性（見 KB-024），但客觀上也同時擋下了頁首病患資訊被誤存為檢驗數據的風險。
- **此規則的驗證範圍有限，需誠實揭露**：A28 規則目前僅在「文字型 PDF」解析路徑（E2-F2）驗證過，而真實樣本 PoC（KB-023）顯示這條路徑的真實覆蓋率僅約 14%——其餘 86% 為純掃描／拍照圖檔，需要 OCR 才能處理，OCR 路徑目前尚未開發（05_BACKLOG 已拍板維持原排程，見 13_ROADMAP）。**OCR 上線時，A28 這類「靠冒號辨識中繼資料列」的規則需要針對 OCR 輸出的文字特性重新驗證是否依然有效**，不能假設對掃描圖檔辨識出的文字也同樣適用（OCR 輸出的斷行、字元辨識錯誤率與 pdfjs 抽取的乾淨文字層性質不同）。
- **原始上傳檔案本身未做去識別化，這點需要與「不留個資」的說法分開講清楚**：使用者上傳的原始掃描檔／照片（`completeUpload()` 存入 Storage 的原始位元組）**完整保存，未經過任何遮蔽、裁切或去識別化處理**——如果原始報告上印有使用者姓名、生日等資訊，這些資訊會原封不動地存在於 Storage 裡的那份原始檔案中。這與「結構化資料庫裡沒有個資欄位」是兩件不同的事，必須分開向王醫師說明，避免「系統完全不留個資」這種過度簡化、不準確的說法。
...
- 已完成處置：`src/app/layout.tsx` 的 `metadata` 匯出補上 `metadataBase: new URL(process.env.NEXT_PUBLIC_SITE_URL ?? "https://health-devkit.zeabur.app")`（無此環境變數時 fallback 至本專案目前唯一的正式站網域，非機密值）；本機與正式站重新驗證，`og:image` 皆正確解析為完整對外網址。**已於正式站二次部署驗證確認修正生效**（commit `0cb2b42`，`RUNNING`＋`/api/health` 200＋真實瀏覽器對 `health-devkit.zeabur.app` 讀取 `og:image` meta tag，實測輸出 `https://health-devkit.zeabur.app/og-image.jpg`，非先前錯誤的 `localhost:8080`）。
...
## KB-044 Hero v2.1 改版：具名推薦確認過程與排版原則取捨（E7-F2）

- 類型：架構決策＋產品決策記錄（Sprint 28，2026-08-08）
- 內容：`16_HERO_V2_DESIGN.md` 定案 Hero 頁視覺升級——新增希波克拉底雕像插畫與王醫師（王健宇醫師，Facebook 品牌名「獅子座的王醫師」）具名推薦文字。本輪記錄兩類決策依據：

  **一、王醫師確認過程**：PO 與王醫師於 2026-08-08 早餐會議當面確認設計文件 §5 三項內容（① 具名介紹方式、② 「與團隊共同發起」的參與程度描述、③ 不放真人照片改以雕像意象代表），三項全數同意、無需調整。這是**當面口頭確認**，非書面簽署，記錄於設計文件供未來查證脈絡；文件同時明確排除「命理／星座聯想」風險——「獅子座」純屬既有品牌命名慣例的一部分，文案刻意不做任何解釋或連結，避免與平台「不用命理」的核心原則（憲法 §3）產生語意衝突。

  **二、排版原則取捨（為何不能當滿版背景）**：PO 明確指示插畫「不可以當滿版背景、不可以把標題文字疊在圖片上」，理由是雕像圖為 900×1499 直向構圖，畫面由上到下佈滿卡片與物件，沒有可疊放文字的留白區域——若當滿版背景疊文字，文字必然與畫面元素衝突，可讀性風險高。改為「獨立插圖區塊」（維持原始比例、寬度限制 500–600px、置中，見 `src/app/page.tsx`）是唯一能同時滿足「保留完整構圖」與「文字可讀」兩個要求的做法。這與 KB-024「誠實地不完整優於自信地錯」同一種審慎精神的視覺設計版本：寧可用較保守的獨立區塊呈現，也不要冒文字疊圖失敗的風險。

- 已完成處置：`src/app/page.tsx` 新增插圖區塊（`next/image`，`alt="希波克拉底雕像，象徵醫者誓言與醫病信任"`）與具名推薦文字區塊，皆置於既有三個信任卡片下方；既有 H1／副標／CTA／信任卡片排版完全不動。單元測試新增覆蓋（`tests/unit/home-page.test.tsx`），過程發現 `next/image` 在 jsdom 測試環境下的用戶端行為（devicePixelRatio 偵測）會在測試環境拆卸後才觸發非同步例外，比照 Next.js 官方測試建議加上 `vi.mock("next/image", ...)` 改用純 `<img>` 通過測試環境（非執行期程式碼變更，僅測試環境設定）。
- 影響範圍：僅 `/` 首頁；不影響 `/privacy`／`/scope`／`/about`／`/ai-principles` 四頁；`tests/unit/home-page.test.tsx` 增加 2 個測試案例（插圖 alt text、具名推薦文字逐字比對）。
- 未來避免：任何未來要在 Hero 或行銷頁面放入「直向長圖、無留白」構圖的視覺素材時，先確認畫面本身是否留有可疊文字的區域，不要預設所有圖片都能當滿版背景疊字——這類判斷應該在設計 Prompt／素材產製階段就先確認，而非等實作時才發現排版衝突（本輪 PO 在需求階段就先明確排除這個做法，屬於「先問不要先猜」的正面案例）。

## KB-045 `/privacy` 頁文案落後於功能：E2-F5 刪除引導提示上線後，隱私頁未同步揭露

- 類型：文案落差修正（Sprint 28 收尾，2026-08-08，內測前發現）
- 內容：`/privacy` 頁「檔案怎麼存放」段落文案定案於 Sprint 25（E7-F1，`14_PUBLIC_SITE_COPY.md`），當時 E2-F5（原始掃描檔刪除引導提示）尚未存在——`deleteDocument()` 雖自 E2-F1 起就能刪除原始檔，但直到 Sprint 26（E2-F5）才在文件列表加上使用者可自主刪除的引導提示，成為使用者實際會操作到的功能。`/privacy` 頁本身在 E2-F5 上線後未回頭同步更新，導致「隱私與資料安全」頁的說明落後於實際功能一個 Sprint，內測前盤點時發現這個落差：頁面沒有告訴使用者「原始檔案其實會保留、且不會自動去識別化、但你可以自己刪除」這件事，與 KB-039（給王醫師的技術現況說明）及 KNOWN_ISSUES.md 第 6 項已經誠實揭露給王醫師與內部文件的內容不一致——公開頁面理應與已知的真實限制保持同步，不能讓內部文件比對外文案更誠實。
- 已完成處置：`/privacy` 頁「檔案怎麼存放」段落新增一句揭露（PO 指定逐字文字，不重寫）：「你確認完數值後，原始檔案（PDF 或照片本身）仍會保留在系統裡，方便之後核對或匯出；如果報告上印有你的姓名等資訊，這份原始檔案不會另外去識別化。如果你不需要保留，可以隨時到「健檔報告」頁面自行刪除，系統不會自動幫你刪。」屬純文案修正，不開新 Feature／不走完整 DOR（比照專案既有「小型文字修正」慣例處理），但仍記錄本則 KB 說明落差成因與修正理由，避免無聲無息地改公開頁面文字。本機驗證：文字逐字比對正確、axe 掃描零 Critical／Serious 違規、桌面與手機寬度皆無版面溢出。
- 影響範圍：僅 `/privacy` 頁一個段落；不影響其他四個公開頁面、不影響任何既有測試斷言（`/privacy` 頁本身無專屬單元測試，僅 e2e 無障礙抽查涵蓋）。
- 未來避免：**任何新 Feature 若會改變既有公開頁面文案已經做出的承諾或揭露範圍（尤其是隱私／資料安全類頁面），應該在該 Feature 的 DOD 檢查表額外加一項「公開頁面文案是否需要同步更新」**，不要只驗收功能本身、假設文案永遠跟得上——本次落差存在了兩個 Sprint（26→28）才被發現，若非內測前主動盤點，可能會更晚才被注意到。KNOWN_ISSUES.md 第 6 項本身已誠實記錄這個限制，但只在內部文件揭露、外部公開頁面未同步，仍算是一種「誠實但不完整」的落差。

## KB-046 品牌命名層：新增「偵醫探心」品牌名稱前綴（純文字層，範圍不含網域與視覺識別）

- 類型：文案更新（Sprint 28 收尾，2026-08-08，比照 KB-045 規模處理，不開新 DOR）
- 內容：PO 決定為平台加上品牌名稱「偵醫探心」，作為既有「個人健康檢查管理平台」描述性名稱前的品牌層——三處純文字替換：`src/components/public/public-shell.tsx`（`PublicHeader()`，五個公開頁面共用頁首，均受影響）、`src/app/page.tsx`（`metadata.title` 與 `metadata.openGraph.title`）、`src/app/layout.tsx`（`metadata.title`，根層級 fallback）。僅文字層命名，本次範圍明確不含網域、Logo 或其他視覺識別（PO 指示：「不要自行擴大改動範圍到 Logo 或其他視覺元素」），這些留待未來另行決定。
- 已完成處置：三處替換完成；`/privacy`／`/scope`／`/about`／`/ai-principles` 四頁本身的 `<title>` 未在範圍內（各頁沿用自己既有標題），但因共用 `PublicHeader()`，五頁頁首連結文字皆已同步更新為「偵醫探心｜個人健康檢查管理平台」。本機驗證：typecheck／lint／213 個單元測試全綠（過程中 `pg-queue-adapter.test.ts` 出現一次孤立失敗，單獨重跑與全套重跑皆綠，判斷為平行執行時共用佇列資料表的既有已知類型瑣事，非本次文字變更所致）、7 個 axe 無障礙抽查零違規、五頁頁首逐一確認品牌名稱正確顯示、首頁 `document.title` 與 `og:title` 皆正確反映新名稱。
- 影響範圍：僅品牌顯示文字（瀏覽器分頁標題、頁首連結、OG 分享卡片標題）；不影響排版、色彩、任何功能邏輯；不影響網域、Logo 或其他視覺識別。
- 未來避免：若未來品牌命名要延伸到網域或視覺識別（Logo、favicon、色彩系統等），應視為獨立 Feature 另開 DOR，不宜比照本次「純文字層」規模處理——本次刻意限縮範圍正是為了避免命名決策蔓延成未經規劃的視覺改版。
