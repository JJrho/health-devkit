...
> 目前狀態：Sprint 13 ✅ 已 commit（`a93fdbd`）＋push＋正式站部署驗證通過（2026-07-19）——**E4-F1：知識來源與檢索基座**。本輪首次使用真實授權內容（王健宇醫師書籍節錄），過程發現並否決一批品質不佳的舊 OCR 內容，並登記新知識點 KB-029（Postgres 內建全文檢索對中文不斷詞）。
