...
- 測試：原計畫將 `DocumentRow`／`DocumentItem` 具名匯出以便直接測試，但 `pnpm build` 失敗——Next.js App Router 的 `page.tsx` 僅允許固定幾種匯出（`default`／`metadata`／`generateStaticParams` 等），任何額外具名匯出都會讓路由型別檢查報錯。改為把提示區塊抽成獨立元件 `src/components/documents/deletion-guidance-notice.tsx`（`DeletionGuidanceNotice`），`DocumentRow` 內部呼叫，`page.tsx` 匯出面維持不變。新增 `tests/unit/deletion-guidance-notice.test.tsx`（AC-1／AC-3：文案顯示、點擊呼叫 `onFocusDeleteButton` callback）。過程發現 Vitest 未啟用 `globals`，`@testing-library/react` 自動 cleanup 偵測不到全域 `afterEach`，改為手動 `afterEach(cleanup)`，避免多個 `it()` 之間 DOM 累積導致誤判「找到多個元素」。
- 全專案 211 個測試（+2）／typecheck／lint／`pnpm build` 全綠。

### KB 補充
- KB-037：og:image 完成記錄
- KB-038：不做自動刪除的決策依據
- KB-039：給王醫師的技術現況說明（結構化表無 PII 欄位、A28 排除規則、14% 覆蓋率限制、原始檔未去識別化的誠實揭露）
- KB-040：正式站部署驗證中發現並修正的真實缺陷（詳見下方）

### 驗證範圍誠實說明
- **og:image**：本機瀏覽器直接驗證通過——`/og-image.jpg` 可正常存取（1200x630）、Hero 頁 `og:image` meta tag 正確指向該路徑。
- **E2-F5 刪除引導提示**：`DeletionGuidanceNotice` 元件本身已用 Testing Library 單元測試驗證文案與點擊行為（AC-1／AC-3）；**AC-2（非 confirmed 狀態不顯示）與完整登入後的真實瀏覽器端到端驗證本輪未執行**——需要真實帳號＋專案＋上傳文件＋Worker 解析＋人工確認完整走一輪才能在瀏覽器中重現 `confirmed` 狀態，而本機 `pnpm dev` 未同時啟動 Worker（需另開 `pnpm worker`），本輪評估此變動風險低（純文案＋單一布林條件渲染，不改動任何服務層邏輯）而未額外搭建完整流程。**PO 2026-08-05 指示：下次有真實使用者走過上傳流程時順手確認一次並記錄結果**（已列入 KNOWN_ISSUES.md 第 8 項）。

### 正式站部署驗證（2026-08-05）
`f32945f` push 後 web service 自動重建，約 8 分鐘後轉 `RUNNING`；`/api/health` 200。依 KB-025 不僅看這兩者就結案，用真實瀏覽器對正式站真實網域（`health-devkit.zeabur.app`）驗證：`/og-image.jpg` 可正常存取（1200x630）。**驗證過程發現並當場修正一項真實缺陷（KB-040）**：Hero 頁 `og:image` meta tag 實際輸出 `http://localhost:8080/og-image.jpg`——不是正式站網域，而是容器內部伺服器位址，外部服務完全無法抓取。本機驗證階段完全沒有現形（本機 dev server 對外/對內位址剛好一致），只有在正式站環境才會暴露。根因是 Next.js Metadata API 的相對路徑圖片欄位需要 root layout 設定 `metadataBase` 才能解析成絕對網址。已於 `src/app/layout.tsx` 補上 `metadataBase: new URL(process.env.NEXT_PUBLIC_SITE_URL ?? "https://health-devkit.zeabur.app")`，本機與正式站重新驗證皆正確解析為完整對外網址（`https://health-devkit.zeabur.app/og-image.jpg`）。此修正已一併 commit／push，觸發第二次部署並確認正式站 RUNNING、`og:image` meta tag 正確。

### 下一步
無（本輪已結案）。E2-F5 完整端到端驗證待下次真實上傳流程時補做，見 KNOWN_ISSUES.md 第 8 項。

## Sprint 25 — E7-F1：公開站五頁（Hero／說明／能做不能做／隱私／來源與AI原則）✅ 實作＋測試＋本機與正式站瀏覽器驗證皆完成，正式結案

- 期間：2026-08-05（單日完成）
- DOR：✅ 通過（sprints/sprint-25-dor.md；A149–A151 由 PO 追認）
- 目標：全案 20 個原始 Feature 上線後，PO 與 Claude Chat 另外定案公開站五頁文案（`14_PUBLIC_SITE_COPY.md`，交接清單 `15_HANDOFF_PUBLIC_SITE.md`），取代 `src/app/page.tsx` 過渡版 Hero，補齊上游 §6.1 公開網站六項中 MVP 開發期間一直缺席的四個頁面。

### 設計要點
- **文字逐字採用文案檔，不重寫不潤飾**：五頁 H1／內文／CTA 皆與 `14_PUBLIC_SITE_COPY.md` 逐字比對通過（本機瀏覽器 `get_page_text` 實測核對）。
- **`/scope` 合併為單頁兩區塊，不拆兩個路由**（A149）：文案檔本身即以單一 H1＋兩區塊呈現，15_HANDOFF 明示兩種做法皆可；本專案無跨頁共用導覽元件可參照慣例，選擇與文案結構一致的單頁呈現。
- **新增公開頁共用外殼**（`src/components/public/public-shell.tsx`，A150）：簡易頁首（站名連回 `/`）＋簡易頁尾（四頁互連導覽），純導覽結構、非文案內容——五頁若無法互相導航，使用者離開 Hero 頁信任區塊後無法回頭，不符合基本可用性。
- **色彩沿用既有慣例**：全數使用既有 `blue-700`／`slate` 色階（與 `AuthCard`／`SubmitButton` 等既有元件一致），未新增色彩 token 或色彩系統。
- **OG meta**：`og:title`／`og:description` 已加在 `/` 之 `metadata` 匯出並實測正確輸出（26／45 字，皆在文案檔建議上限內）；`og:image` 本輪不提供（A151，PO 確認，無圖片素材）。

### 驗收結果
- AC-1～AC-7 全數通過：五頁文字逐字核對、Hero 三個信任區塊連結正確、色彩未新增系統、五頁 axe-core 掃描零 Critical／Serious 違規（含既有首頁／註冊／登入頁回歸）、375px 行動裝置寬度五頁皆無水平溢出、OG meta 格式與字數正確、五頁未登入皆可直接訪問（200）。
- 全專案 209 個測試（+1，修正 `tests/unit/home-page.test.tsx` 因 Hero 文字變更而過期的斷言，並新增信任區塊連結測試）／typecheck／lint／`pnpm build` 全綠。

### 正式站部署驗證（2026-08-05）
`c8d7eb0` push 後 web service 自動重建，約 8 分鐘後轉 `RUNNING`；`/api/health` 200。依 KB-025 不僅看這兩者就結案，用真實瀏覽器對正式站真實網域（`health-devkit.zeabur.app`）逐一開啟五個新公開頁面，內容與本機驗證結果一致。OG meta 於正式站實測正確輸出（`og:title`／`og:description`）；另用第三方 OG 除錯工具 `opengraph.xyz` 對正式站網址掃描（本機環境無法直接在 LINE／Messenger 應用程式內測試，改用運作原理相同的公開工具驗證，見 KB-037 說明），確認標題／描述皆正確解析、無截斷，`og:image` 依 A151 預期缺失（非破圖）。**PO 已確認 `og:image` 列為部署後近期待辦**，已同步列入 `KNOWN_ISSUES.md` 第 9 項。完整結果見 KB-037。

### 下一步
無（本輪已結案）；`og:image` 待 PO 提供圖片素材後低成本追加，見 KNOWN_ISSUES.md 第 9 項。
...
## 補充：E4-F1 知識庫真實內容擴充第二批（2026-07-24，非獨立 Sprint，延續 A55 既定流程）

- 性質：內容資料補充，非新 Feature／新 Sprint，與同日稍早的第一批擴充手法相同，同樣僅異動 `scripts/seed-knowledge-sources.ts`。
- 內容來源：PO 再提供王健宇醫師書籍 PART2 另外 4 個章節掃描 PDF（〈感冒、過敏分不清？〉頁 062–073、〈摸到不明腫塊，是癌症？〉頁 074–081、〈腰痛，看骨科、神經科還是復健科？〉頁 088–096、〈腫瘤標記異常，有癌症？〉頁 113–120，共 38 頁），比照既定方法（AI 逐頁視覺閱讀＋人工轉錄，非自動 OCR）轉錄。
- 安全處置：延續 A55 既定原則，四個新章節 `status` 皆設為 `draft`；已直接查詢共用資料庫確認安全閘門對新內容同樣生效。
- 結果：`knowledge_sources` 由 10 筆增至 14 筆王健宇醫師章節，`knowledge_chunks` 共 396 筆（原 270 筆＋新增 126 筆），皆為 `status=draft`。全專案 208 個測試／typecheck／lint／`pnpm build` 全綠（測試數量不變，純內容擴充）。同樣因共用同一 Supabase 資料庫，本機 seed 後正式站即同步生效。
- 已知限制：現為 14/38 章節（37%），其餘 24 章節（約 613 頁）留待後續迭代；轉錄品質限制同前批。

## 補充：E4-F1 知識庫真實內容擴充第三批（2026-07-24，非獨立 Sprint，延續 A55 既定流程）

- 性質：內容資料補充，非新 Feature／新 Sprint，手法與前兩批相同，同樣僅異動 `scripts/seed-knowledge-sources.ts`。
- 內容來源：PO 提供王健宇醫師書籍 PART3（別忽略小毛病後面的大問題）4 個章節掃描 PDF（〈喝止咳藥水，咳嗽還是停不了？〉頁 122–132、〈吃了止痛藥，頭還是痛？你需要寫日記！〉頁 133–146、〈便秘、腹瀉、排便異常，吃藥就好了？〉頁 147–153、〈胃堵堵、胃糟糟，你只靠胃藥搞定？〉頁 154–161，共 40 頁），比照既定方法（AI 逐頁視覺閱讀＋人工轉錄，非自動 OCR）轉錄。本批含兩份自我記錄表格頁（頭痛日誌、咳嗽紀錄表），以文字描述其欄位結構呈現，非逐格複製版面。
- 安全處置：延續 A55 既定原則，四個新章節 `status` 皆設為 `draft`；已直接查詢共用資料庫確認安全閘門對新內容同樣生效。
- 結果：`knowledge_sources` 由 14 筆增至 18 筆王健宇醫師章節，`knowledge_chunks` 共 523 筆（原 396 筆＋新增 127 筆），皆為 `status=draft`。全專案 208 個測試／typecheck／lint／`pnpm build` 全綠（測試數量不變，純內容擴充）。同樣因共用同一 Supabase 資料庫，本機 seed 後正式站即同步生效。
- 已知限制：現為 18/38 章節（47%），其餘 20 章節（約 573 頁）留待後續迭代；轉錄品質限制同前兩批，PART3 部分頁面（如頁 124–131、138–140）文字密度高、直排多欄版面判讀難度較高，轉錄時已盡力去除明顯重複片段以求語句通順，但逐句對應原文頁面邊界的精確度低於前兩批相對單純的敘事型內容。

## 補充：E4-F1 知識庫真實內容擴充第四批（2026-07-24，非獨立 Sprint，延續 A55 既定流程）

- 性質：內容資料補充，非新 Feature／新 Sprint，手法與前三批相同，同樣僅異動 `scripts/seed-knowledge-sources.ts`。
- 內容來源：PO 提供王健宇醫師書籍 PART3 另外 4 個章節掃描 PDF（〈腰背疼痛，小心結石惹禍！〉頁 162–168、〈頭暈眼花，哪裡有問題？〉頁 169–173、〈發燒當感冒，竟是敗血症？〉頁 174–182、〈體重突然減輕，別高興！〉頁 183–189，共 35 頁），比照既定方法（AI 逐頁視覺閱讀＋人工轉錄，非自動 OCR）轉錄。
- 安全處置：延續 A55 既定原則，四個新章節 `status` 皆設為 `draft`；已直接查詢共用資料庫確認安全閘門對新內容同樣生效。
- 結果：`knowledge_sources` 由 18 筆增至 22 筆王健宇醫師章節，`knowledge_chunks` 共 616 筆（原 523 筆＋新增 93 筆），皆為 `status=draft`。全專案 208 個測試／typecheck／lint／`pnpm build` 全綠（測試數量不變，純內容擴充）。同樣因共用同一 Supabase 資料庫，本機 seed 後正式站即同步生效。
- 已知限制：現為 22/38 章節（58%），其餘 16 章節（約 538 頁）留待後續迭代；本批「頭暈眼花」〈頁 171–173〉與「發燒當感冒」〈頁 180〉頁面文字密度同樣偏高，轉錄時已去除明顯重複片段，精確度限制同前批說明。

## 補充：E4-F1 知識庫真實內容擴充第五批（2026-07-24，非獨立 Sprint，延續 A55 既定流程）

- 性質：內容資料補充，非新 Feature／新 Sprint，手法與前四批相同，同樣僅異動 `scripts/seed-knowledge-sources.ts`。首次涵蓋 PART4（最常發生的醫藥問題 Q&A）章節。
- 內容來源：PO 提供王健宇醫師書籍 PART4 4 個章節掃描 PDF（〈為什麼醫生要轉診？〉頁 201–205、〈吃藥，會不會有副作用？〉頁 213–219，含一頁藥物過敏紀錄卡表格頁 218、〈健康檢查越貴，數據越精準？〉頁 220–225、〈西醫好？還是中醫好？〉頁 226–228，共 30 頁），比照既定方法（AI 逐頁視覺閱讀＋人工轉錄，非自動 OCR）轉錄；表格頁（頁 218）以文字描述其欄位結構呈現，非逐格複製版面。
- 安全處置：延續 A55 既定原則，四個新章節 `status` 皆設為 `draft`；已直接查詢共用資料庫確認安全閘門對新內容同樣生效。
- 結果：`knowledge_sources` 由 22 筆增至 26 筆王健宇醫師章節，`knowledge_chunks` 共 689 筆（原 616 筆＋新增 73 筆），皆為 `status=draft`。全專案 208 個測試／typecheck／lint／`pnpm build` 全綠（測試數量不變，純內容擴充）。同樣因共用同一 Supabase 資料庫，本機 seed 後正式站即同步生效。
- 已知限制：現為 26/38 章節（68%），其餘 12 章節留待後續迭代；本批「吃藥，會不會有副作用？」頁 219 文字密度偏高，轉錄時已去除明顯重複片段，精確度限制同前批說明。

## 補充：E4-F1 知識庫真實內容擴充第六批（2026-07-24，非獨立 Sprint，延續 A55 既定流程）

- 性質：內容資料補充，非新 Feature／新 Sprint，手法與前五批相同，同樣僅異動 `scripts/seed-knowledge-sources.ts`。本批補齊 PART4（最常發生的醫藥問題 Q&A）剩餘 4 個章節，**PART4 全 8 章節至此全數轉錄完成**。
- 內容來源：PO 提供王健宇醫師書籍 PART4 剩餘 4 個章節掃描 PDF（〈想找好醫生，看名醫排行榜準沒錯？〉頁 197–200、〈藥多吃一點，比較有效？〉頁 206–212、〈規則服藥，真的不能停嗎？〉頁 192–196、〈無底洞療法，無效又傷身！〉頁 229–234，共 27 頁），比照既定方法（AI 逐頁視覺閱讀＋人工轉錄，非自動 OCR）轉錄。
- 安全處置：延續 A55 既定原則，四個新章節 `status` 皆設為 `draft`；已直接查詢共用資料庫確認安全閘門對新內容同樣生效。
- 結果：`knowledge_sources` 由 26 筆增至 30 筆王健宇醫師章節，`knowledge_chunks` 共 749 筆（原 689 筆＋新增 60 筆），皆為 `status=draft`。全專案 208 個測試／typecheck／lint／`pnpm build` 全綠（測試數量不變，純內容擴充）。同樣因共用同一 Supabase 資料庫，本機 seed 後正式站即同步生效。
- 已知限制：現為 30/38 章節（79%），其餘 8 章節（分屬 PART1／PART2／PART3 未轉錄部分）留待後續迭代；本批「規則服藥，真的不能停嗎？」頁 194–196 文字密度偏高，轉錄時已去除明顯重複片段，精確度限制同前批說明。
