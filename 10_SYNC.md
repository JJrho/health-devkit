...
## 4. 下一步
**E4-F3 PoC 1/2 實作＋測試完成**，待 PO 確認 commit／push／正式站部署時機；部署驗證通過後即可開 Sprint 16 DOR（依既定順序推進 E4-F3 PoC 2/2：UI／`regenerate`／頻率限制／進階安全打磨）。剩餘 32 章節（約 681 頁）真實內容轉錄方式待決定（非阻塞）；KB-018／KB-020 待 PO 決定處理時機（非阻塞）。

## 5. 最高優先事項
Sprint 15（E4-F3 PoC 1/2）commit／push／正式站部署驗證；之後 Sprint 16 DOR 規劃（E4-F3 PoC 2/2）；剩餘 32 章節（約 681 頁）真實內容的轉錄方式待決定（非阻塞）；KB-018（RLS BYPASSRLS）與 KB-020（E1-F2 登入問題，C6 牴觸）待決定處理時機。
