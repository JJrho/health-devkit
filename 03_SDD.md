...
**E1-F3 Google 登入與帳號連結模組完成，正式結案（2026-07-21，Sprint 21，Feature 18/20）**：補上第二種登入方式，E1 Epic 全數完成（5/5）。核心設計：走 Supabase Auth 內建 OAuth 供應商代理，非自架 OAuth client（A121）；瀏覽器端導向流程——`signInWithOAuth()` 導向 Google→回 `/auth/callback`→取得 `access_token`→POST `/api/auth/google` 由伺服器驗證並建立本系統自有 session（A122）；Google 失敗（token 驗證失敗或新帳號未附同意條款）不建立半完成帳號，逐字落實上游 §28.1（A123）；伺服器端驗證 access_token 改用 `anon.auth.getUser()`，不用 Admin API（本專案 `service_role` 為新版不透明金鑰格式，已知不被 Admin API 接受，見 KB-009／KB-012，A124）；同意條款把關集中於註冊頁，登入頁按鈕不顯示勾選框，全新帳號經登入頁嘗試時回 `CONSENT_REQUIRED`（A125）。全專案 184 個測試（+7）／typecheck／lint／`pnpm build` 全綠。已 commit（`e3c5f5d`）＋push＋**正式站部署驗證通過（含真實 Google 帳號端到端登入）**：web／worker 兩 service 皆 `RUNNING`、`/api/health` 200；PO 於 Supabase Dashboard 完成 Google Provider 設定（過程排查發現「Enable Sign in with Google」總開關與填憑證是兩個獨立步驟，漏開開關導致初次測試仍回 `provider is not enabled`，修正後即正常）；PO 用本人真實 Google 帳號完整跑通一次登入流程——導向 Google 真實登入畫面→完成帳戶驗證→同意授權→正確導回正式站並成功落地 `/projects`，確認 session 建立與四層權限鏈存取皆正常，A122／A124 在正式生產環境完整驗證。**已知非阻塞待辦**：Google 同意畫面顯示 Supabase 專案網域而非友善應用程式名稱，需 PO 於 Google Cloud Console 另行設定，純品牌體感問題不影響功能。

**E5-F4 看診摘要與資料匯出模組完成，正式結案（2026-07-22，Sprint 22，Feature 19/20）**：E5 健康行動閉環全數完成（4/4），M4 里程碑達成。核心設計：看診摘要（C19）不建立持久化物件，即時彙整既有資料（A128）；**A129 實作中修正**——原計畫用 `pdf-lib` 伺服器端繪製 PDF，實測發現其標準字型不支援中文，改為瀏覽器友善列印方案（`@media print` A4 排版＋`window.print()`），不需額外字型或套件；資料匯出（C20）範圍為單一專案（A130），ZIP 封裝用 `archiver@7.0.1`（實作中發現 v8 API 不相容，釘選 v7，A131）；原始檔透過既有 `StorageAdapter` 直接串流進 ZIP 回應，不簽發對外連結（A132，本專案首個直接串流二進位回應的端點）；「想問醫師的問題」純自由文字不落地（A133）；「執行中計畫」區塊銜接 E5-F3——近期有「需要專業評估」檢討的計畫附註提示並原樣帶出既有轉介摘要內容。全專案 192 個測試（+8）／typecheck／lint／`pnpm build` 全綠。已 commit（`96ea2ec`）＋push＋**正式站部署驗證通過**：web／worker 兩 service 皆 `RUNNING`、`/api/health` 200；用 Admin API 建立正式站測試帳號，真實瀏覽器對正式站完整操作驗證——建立計畫並完成一次「需要專業評估」的檢討＋產生轉介摘要→至 `/projects/{id}/reports` 產生看診摘要，確認「執行中計畫」區塊正確附註提示並原樣帶出轉介摘要內容，驗證 A128 銜接 E5-F3 在正式生產環境成立→直接呼叫匯出端點確認回應 200／`application/zip`／UTF-8 中文檔名／1787 bytes，驗證 A130／A132 在正式站成立，驗證用測試帳號與資料已全數清除。

**E6-F1 稽核事件與刪除鏈模組實作＋測試＋本機瀏覽器驗證完成（2026-07-22，Sprint 23，Feature 20/20，尚未 commit）**：E6 稽核基座第一個 Feature。新增 `audit_events` 表（`userId` 不設外鍵，A138，稽核紀錄須在帳號永久刪除後仍可查詢追溯）取代 A11 過渡期 `logger.warn` 作法；帳號刪除採三十日冷靜期，重用既有 `QueueAdapter.enqueue()` 的 `runAt` 延遲排程機制，不新增排程基礎設施（A135）；撤銷不直接操作 `queue_jobs` 列，改由到期執行前重新檢查 `deletionRequestedAt`（A136，防競態）；正式永久刪除 FK 順序泛化自 `cleanupTestData()`（A139），**實作中額外發現並修正該測試輔助函式本身遺漏的缺口**——`consent_records` 對 `users` 為 `ON DELETE no action`，正式帳號永久刪除若不先清空會直接外鍵違反失敗。全專案 201 個測試（+9）／typecheck／lint／`pnpm build` 全綠；本機瀏覽器完整操作驗證通過（申請→倒數提示→撤銷→UI 正確復原；未登入呼叫回 401）。**誠實記錄**：AC-3／AC-4（背景到期永久刪除、防競態）以直接呼叫處理器模擬到期驗證，未實測真實等待 30 天。
